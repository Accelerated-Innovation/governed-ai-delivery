...
The profile is planned at .govkit/profile.yaml, the generated resolution at .govkit/resolution.json, and the marker remains backward compatible. Do not imply those new files already exist. Decide ownership and trust boundaries for workflow/evidence records in I06–I07.
...
- [ ] New profiles require no maturity level and can express independent capability combinations.
- [ ] A repository can permit a qualifying small-change workflow and a full Gherkin workflow without profile edits between requests.
- [ ] Unknown repository/stack characteristics can be represented without accepting a framework default; relevant required decisions remain explicit.
- [ ] Observations cannot silently become accepted policy; existing authoritative documents can be referenced without duplication.
- [ ] Scoped retain/improve/migrate decisions can coexist; recording a target architecture does not activate it everywhere.
- [ ] Mandatory policy controls remain required regardless of a request's preferred workflow.
- [ ] Preview has no target writes and distinguishes accepted decisions, proposals, and unknowns.
- [ ] Profiles and resolution records are schema-valid, versioned, and deterministic.
- [ ] Invalid/conflicting choices produce actionable errors; legacy marker, flags, upgrades, and edit protection remain covered.

- [ ] Maintenance policy can express approved sources, channels, pins/compatibility constraints, and freshness limits without making online lookup mandatory or changing the project lock during assessment.
- [ ] A newer release alone does not produce a policy violation; missing/stale release information is represented explicitly rather than treated as current.
...
- [ ] The package contract supports both a skill-only pack and a pack containing skills, configuration/resources, and executable controls.
- [ ] Skills are available through each supported agent's appropriate native mechanism from one authoritative package source; required resources resolve correctly.
- [ ] A pack's required controls run through local/CI tooling without an agent session or loaded skill; agent self-report alone cannot satisfy enforcement.
- [ ] Resolution is deterministic and side-effect free; cycles/conflicts fail before copying.
- [ ] Independent combinations include LLM evaluation with application governance without mandatory Gherkin artifacts, and Gherkin delivery without LLM capabilities.
- [ ] Every required dependency has an explicit rationale; the new resolver does not infer dependencies from a maturity label.
- [ ] All shipped packs resolve; `govkit_min_version`, compatibility, versions, and digests are enforced.
- [ ] Available, installed, applicable, and policy-required states remain distinct. Selecting fewer optional skills/packs cannot remove mandatory controls.
- [ ] Brownfield adoption reuses existing accepted sources and installs only necessary integrations/resources; customization of the full exemplar library is not required.
- [ ] Exemplars do not become authoritative through installation, and project-owned decisions/overrides remain distinct from shared pack content.
- [ ] Pinned resources resolve on a clean developer/CI environment after explicit setup, support offline request resolution, and fail clearly if required content is missing or mismatched.
- [ ] Preview performs no target writes and shows instruction/CI changes and ownership; authorized application is idempotent and preserves local edits.
- [ ] Proposed removals expose affected dependencies/controls and cannot silently weaken policy.
- [ ] Existing extension behavior remains covered through compatibility; no mandatory new registry or silent layout migration is introduced.
- [ ] Worked examples demonstrate the LLM evaluation pack and a skill-only pack across installation, invocation, and applicable independent checks.
...
- [ ] Missing, unreadable, unconfigured, unknown, or skipped required evidence never becomes a silent pass.
- [ ] Findings/results validate against versioned schemas, preserve provenance, and use stable identifiers.
- [ ] Checks do not print/exit internally; compatibility and independent failure reporting are covered.
- [ ] Representative controls include a demonstrated failing case as well as a passing case.
