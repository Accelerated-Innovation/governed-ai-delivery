...
Status: I00–I12 merged; I10 external-caller acceptance remains open; I13a PR #199 has both Qodo findings fixed and pushed; automatic attribution/CI pending, I13b inputs remain open.
Plan version: 1.41.
Prepared: 2026-09-23.
...
| I13 | in progress | [I13a PR #199](https://github.com/Accelerated-Innovation/governed-ai-delivery/pull/199), implementation `a32a56e3d38222f58a0eb931996af3d9e22b994c`: capability onboarding, executable recipe and actual skill inventory; twenty new cases, 4,712 broad / 28 final focused / 260 legacy passes plus wheel checks; two Qodo fixes `3aec4ed` add nineteen cases and 4,735 final fast-suite passes | I13b requires maintainer release/warning decision, real pilot observations and protected-caller evidence; no retirement scheduled |

Current handoff:

- Main synchronized to PR #198 merge `fcb0b1119b8c7b13703979b9622b9e5404c265fc`. Remote branch deletion verified; tree-identical local branch removed. Forty-eight known I12/PR198 temporary artifacts removed; repository `.venv` preserved.
- I13a is delivered in [PR #199](https://github.com/Accelerated-Innovation/governed-ai-delivery/pull/199), implementation `a32a56e`, branch `feat/149-release-readiness`: README leads with explicit profiles/packs and references an executable onboarding guide; existing marker users retain migration/rollback and the legacy bundle path. No consumer bundle is installed into GovKit source.
- Test first: sixteen recipe cases failed before the guide existed; four tutorial inventory cases expose three nonexistent skills and omitted PR-author documentation. Recipe tests exercise all three agents/both provider selections, read-only previews, metadata-only apply, idempotency, selected native skills, preservation and real pass/fail controls. Final focused checks pass **28 cases (9.44s)**; frozen legacy compatibility passes **260 cases (3.94s)**. Clean-wheel verification passes all sixteen recipe cases, twenty-one pack installs and three migration/rollback scenarios. Broad fast validation passed **4,712 tests, 2 existing skips and 150 e2e deselected (482.80s)**; the four later inventory cases pass in the final focused run.
- I12 is integrated, including both review fixes and final-head hosted checks. Source self-hosting preserves unknown architecture and protected-caller evidence. #148 remains closed; #147/#149/#142 remain open.
- No removal release or warning period is announced. New onboarding does not start a deprecation clock; legacy commands/manifests remain supported. Requested product input may refine a proposed boundary without blocking I13a.
- The known Qodo local-review integration blocker (`repo_not_connected` / `MT-WORKSPACE-NO-REPOS` / HTTP 403 `WORKSPACE_HAS_NO_AUTHORIZED_REPOS`) is not retried until support/reconnection confirmation. Structured rules retrieval works and was refreshed for I13a. No clean local review is inferred.
- Review remediation: both PR #199 findings reproduced, fixed and pushed as `3aec4ed`. GovKit capability skills use their reserved native namespace with patch-versioned pack updates and protected migration; the published-package quickstart calls only `--help` before checking available commands. Nineteen new cases initially produced 16 failures / 3 passes; 157 focused checks pass. Fresh-wheel verification passes; the final broad suite passes **4,735 tests, 2 existing skips and 150 e2e deselected (332.49s)**. All four pre-fix final-head jobs passed in Tests run `36069075155`.
- Next action: review/integrate PR #199 with fixes `3aec4ed`; Qodo attribution and hosted fix-head CI remain pending. Plan and issue delivery records remain current. Do not merge without separate authorization. I13b and full release-candidate completion retain actual pilot/provider evidence and the explicit retirement decision.
- Open product input: FeaturePeers definition, protected caller deployments, representative team repositories, pilot participants and legacy-removal release remain separate later inputs.
