...
Status: I00–I11 merged; I10 external-caller acceptance remains open; I12 PR #198 has two Qodo findings remediated and pushed; Qodo attributes both as implemented, integration pending.
Plan version: 1.39.
Prepared: 2026-09-23.
...
| I12 | in progress | Minimal source profile/empty lock, real source checks and canonical posture; twenty new tests, 4,676 fast passes, 51 final focused passes, actual source and installed-wheel proof | [PR #198](https://github.com/Accelerated-Innovation/governed-ai-delivery/pull/198), implementation `42fdccc`; local demonstration complete, integration pending. Two Qodo review fixes have twenty additional regressions/controls and 71 focused and 4,696 fast-suite passes; fixes `a113181` pushed, both Qodo attributions implemented. I10 external enforcement remains unknown |
| I13 | not started | — | Pilot observations may begin at I07 |

Current handoff:

- Integrated: PR #197 merged as `3ab9f39d13f02d752cc29321ee536bc70428b2fc`, tree-identical to final head `5427be93b793a82794c71b85319e27c626e504eb`. [Final-head Tests run 36052454937](https://github.com/Accelerated-Innovation/governed-ai-delivery/actions/runs/36052454937) passed both Python fast suites, e2e/toolchains and wheel-smoke. All eleven #148 criteria are integrated and #148 is closed; merge/CI does not establish an exact-head clean Qodo verdict.
- Cleanup: synchronized main, pruned the deleted remote branch, removed the verified squash-merged local branch and 34 inventoried I11c temporary entries. Preserved `.venv` (Python 3.12.14 / GovKit 0.21.1).
- Current increment: I12 [PR #198](https://github.com/Accelerated-Innovation/governed-ai-delivery/pull/198) on `feat/149-source-self-hosting`, implementation `42fdcccd6f3b0b2c86275432508fcdc2437b9f62`, based on `3ab9f39`. [ADR 0017](decisions/0017-limited-source-self-hosting.md) and [source guide](SOURCE_SELF_HOSTING.md) define minimal source metadata, always-required local controls, explicit execution, separate reviewed policy and canonical unknowns. No consumer pack, native skill or legacy marker is installed.
- Review limitation: the known local-review integration blocker (`repo_not_connected` / `MT-WORKSPACE-NO-REPOS` / HTTP 403 `WORKSPACE_HAS_NO_AUTHORIZED_REPOS`) is not retried until support/reconnection confirmation. Structured rules retrieval works; I12 standards were refreshed. No clean review is inferred.
- Issue tracking: #148 closed; #147/#149/#142 remain open. I12's self-hosting criteria are locally demonstrated pending integration. #147 still needs protected-caller/path-coverage deployment evidence; #149 retains the concrete compatibility/warning/removal boundary. The voluntary pilot protocol is not collected adoption data.
- Validation so far: twenty added tests pass, with **51 final focused passes (5.21s)** including the existing 31 profile tests. The real source checkout and the fresh installed wheel both resolve/validate the model, execute both configured controls, preserve input identity and replay human/JSON posture. Both retain architecture and protected-caller unknowns. Seven existing runtime-only wheel change pilots pass. The full fast suite passed **4,676 tests, 2 existing skips and 150 e2e deselected (319.53s)**. The final focused run and installed-wheel source smoke cover the subsequently added CI step. Scoped lint/format/whitespace checks pass; all fourteen non-plan paths are recorded for delivery verification.
- Review remediation: both PR #198 findings are fixed and pushed as `a113181`: require a full caller-supplied PR/push comparison base and bind setup-python to the matrix. Twenty additional regressions/controls failed **19 / passed 1** before fixes; **71 final focused checks pass**. A fresh installed-wheel smoke inspected all sixteen changed paths against `3ab9f39`, retaining the expected unknowns. The full fast suite passed **4,696 tests, 2 existing skips and 150 e2e deselected (340.34s)**. Qodo attributes both findings as implemented; the latest completed review remains the original run, so no new clean-head review is inferred. The current hosted baseline `8af260c` passed all four jobs in Tests run `36056976054`; this does not certify unpushed fixes.
- Next action: review/integrate PR #198. After integration, reconcile I13 release hardening and remaining external inputs; do not silently choose a retirement release or claim pilot/provider evidence. Do not merge without separate authorization.
