...
1. Architecture Preflight → run `/govkit-architecture-preflight`
2. GenAI Preflight → run `/govkit-genai-preflight` (validates L5-specific decisions)
3. ADR creation (if required by preflight)
4. Plan finalization → run `/govkit-spec-planning`
5. Evaluation Suite Planning → run `/govkit-eval-suite-planning` (plans configured quality/adversarial/retrieval evaluators suites)
...
| LLM gateway | `LLM_GATEWAY_CONTRACT.md` | provider-neutral port, logical routing, resilience, usage, and budgets |
| Guardrails / safety | `MODEL_GUARDRAILS_CONTRACT.md` | input, context, output, and tool-call policy with fail-closed behavior |
| Observability / telemetry | `LLM_OBSERVABILITY_CONTRACT.md` | privacy-aware telemetry, immutable provenance, usage, and trace correlation |
...
- **Observability via LLM telemetry** — see `LLM_OBSERVABILITY_CONTRACT.md`
