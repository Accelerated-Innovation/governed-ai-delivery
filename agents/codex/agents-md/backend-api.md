# Governed AI Delivery — Foundations (Level 3) — Codex
...
- Path-scoped layer rules via nested `AGENTS.md` files
...
- Follow API naming, versioning, and error rules from
  `docs/backend/architecture/API_CONVENTIONS.md`; update OpenAPI definitions
  when APIs change
...
implementation proceeds. Invoke `$govkit-adr-author` to scaffold a new ADR.
