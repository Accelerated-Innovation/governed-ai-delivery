...
| PDG authority | pdg-authority-gate | `govkit verify-contract` checks **every** commitment in the repository — `commitments/<key>/baseline.json` — for both local drift and current authority, so a code change whose contract files were untouched is still gated, and a repository with four commitments does not gate one of them. The same command with the same flags runs on both providers, which is what makes their outcomes equivalent rather than merely similar. `--base-ref` lets it refuse a commitment deleted from the tree while the PDG still says it authorizes work: retirement is invalidation, not `rm`. Fails closed on drift, on withdrawal, and when the answer cannot be obtained — a gate that passes while the PDG is unreachable is not a gate. **Install only if you have a PDG**; without one, leave it out and `govkit validate-baseline` remains fully useful on its own |
