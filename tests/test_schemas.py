...
# The generic extension-manifest schema is a framework asset shared by every
# extension; it lives under governance/schemas/ alongside the other schemas.
EXT_SCHEMA_PATH = REPO_ROOT / "governance" / "schemas" / "extension-manifest.schema.json"
...
class TestMarkerSchemaRejectsInvalid:
    ...
    def test_accepts_marker_with_extension_sourced_assumption(self):
        ...
        marker = self._valid_marker()
        marker["assumptions"] = [{
            "id": "extension.skill-oriented-agent-architecture.agent-runtime",
            "value": "true",
            "source": "extension",
            "confidence": "high",
            "evidence": ["extensions/skill-oriented-agent-architecture/manifest.yaml"],
            "files_affected": [],
            "review_required": False,
