...
| `govkit posture export` | Project a saved maintenance assessment into privacy-filtered human/JSON reports; explicitly publish a new local artifact. See [posture reporting](docs/POSTURE_REPORTING.md). |
| `govkit posture change` | Project saved change results into privacy-filtered workflow/control/evaluation reports. See [change posture](docs/CHANGE_POSTURE.md). |
| `govkit posture aggregate` | Summarize an explicit cohort of saved exports with missing coverage and freshness denominators. See [offline aggregation](docs/POSTURE_AGGREGATION.md) and the separate [voluntary team pilot](docs/ADOPTION_PILOT.md). |
...
| `govkit pipeline evidence/assess/upgrade-preview` | Compare explicit provider/runtime exports, assess canonical maintenance and preview selected integration upgrades. Publication and approved metadata refresh are opt-in. See [provider evidence](docs/PROVIDER_EVIDENCE.md). |
