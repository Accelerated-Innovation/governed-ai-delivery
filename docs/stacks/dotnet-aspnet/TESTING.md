...
External dependencies must be mocked with **NSubstitute**.
...
- control randomness via seeding (use `Bogus` with a fixed seed)
- freeze time when testing time-based logic (`TimeProvider` abstraction)
...
- explicit assertions required (use **FluentAssertions**)
...
|---|---|---|
| "The test is too strict, let me loosen it" | Tests define the contract; code must meet it | Fix the implementation |
| "The test is hard to write, let me skip it" | Skipped tests mean untested code | Solve the design problem |
| "The test fails, let me adjust expectations" | Changes the spec to fit broken code | Fix the code |
| "This test is flaky, let me add sleep/retry" | Masks real timing or concurrency bugs | Fix the actual race condition |
| "I'll disable this test until I have time" | Tests can't fail; they're requirements | Enable and fix now |
...
```csharp
[Binding]
public class ResetPasswordSteps(WebApplicationFactory<Program> factory)
{
    private HttpResponseMessage _response = null!;

    [Given("a registered user")]
    public async Task GivenARegisteredUser() { /* seed test user */ }

    [When("the user requests a password reset")]
    public async Task WhenUserRequestsPasswordReset()
    {
        var client = factory.CreateClient();
        _response = await client.PostAsync("/v1/users/reset-password", ...);
    }

    [Then("a reset email is sent")]
    public void ThenAResetEmailIsSent()
    {
...
tests/Bdd/ResetPasswordSteps.cs
tests/Contract/UserApiContractTests.cs
```

Name test methods using the pattern: `MethodName_Condition_ExpectedResult`.
