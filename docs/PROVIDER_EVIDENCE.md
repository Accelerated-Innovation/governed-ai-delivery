...
inconsistent admission inputs fail before project checks run.
...
checks retain each explicit fact. Missing facts, absent age policy, stale/future
...
recollecting an old passing export cannot renew freshness. Generated files and
