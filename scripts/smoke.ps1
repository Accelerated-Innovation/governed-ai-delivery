...
.DESCRIPTION
  ...
  runs `govkit apply`, drops in a minimal hello_world_api feature for L4,
  and runs `govkit validate`. Prints a summary at the end.

  L3: validate short-circuits (no per-feature artifacts).
  L4: full 5-artifact validation against the hello_world_api smoke feature.
  L5: applies and creates the folder; the hello_world_api feature will FAIL
      the L5-specific checks (mode is not llm). That is the expected behaviour
      for this smoke harness -- it confirms L5 checks fire, not that they pass.
...
$featurePlan = @'
# Feature Plan: hello_world_api

## Objective

- Provide a minimal `GET /hello` endpoint that returns a fixed JSON greeting
- Used as a smoke test for govkit scaffolding, validation, and CI gates
- Success: both acceptance scenarios pass; p95 latency < 50ms

## Scope Boundaries

### In scope
- One inbound HTTP route returning a fixed JSON greeting

### Out of scope
- Authentication, personalization, persistence

### Assumptions
- The service is reachable on localhost during tests
...
evaluation_prediction:
  first:
    fast:           { score: 5, evidence: "No network or I/O in unit tests" }
    isolated:       { score: 5, evidence: "Pure handler; no shared state" }
    repeatable:     { score: 5, evidence: "Deterministic string response" }
    self_verifying: { score: 5, evidence: "Assertions on status and body" }
    timely:         { score: 4, evidence: "Tests written before handler" }
    average: 4.8
  virtues:
    working:   { score: 5, evidence: "Both acceptance scenarios automated" }
    unique:    { score: 5, evidence: "Single handler; no duplication" }
    simple:    { score: 5, evidence: "One route, fixed response" }
    clear:     { score: 5, evidence: "Route name matches behaviour" }
...
**Goal**
- Add `GET /hello` returning `{"message": "hello, world"}` with status 200
...
$featurePreflight = @'
# Architecture Preflight: hello_world_api
...
- Single inbound route handler; no domain/adapter split required.
