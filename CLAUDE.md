...
`pipeline_render.py` emits paired reusable entry points; `pipeline_store.py` binds exact preview authorization and protects existing files; `pipeline_runtime.py` verifies runtime/profile/pack pins before the common engine. See ADR 0012 and `docs/PIPELINE_GENERATION.md`. Templates require caller-prepared isolated runtime, trusted policy checkout, request and full base SHA. Keep provider inputs in environment values, absolute interpreter validation and Python isolation. Preserve edit protection, stale-input guards, rollback and explicit execution opt-ins. Configuration currency cannot imply activation/execution/enforcement. I10c retains provider event/admission, maintenance facts and publication; generation must not fetch/install packages or rewrite existing workflows. Keep paired goldens, schemas and three-agent installed-wheel execution pilots aligned.
